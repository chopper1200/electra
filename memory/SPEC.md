...
- `/materiali` catalogo + magazzino: ricerca, filtro categoria, movimenti carico/scarico, badge "sotto scorta", **importazione listino fornitore da Excel/CSV** (modale con drag&drop, mapping colonne mostrato, modello scaricabile), **ricarico prezzi** (percentuale su categoria con anteprima prima/dopo) e **carico rapido** (quantità in blocco, filtro sotto scorta, "Riempi mancanti", spesa stimata)
- `/spesa` **Lista della spesa unica**: KPI (cantieri aperti, articoli, da acquistare), tabella desktop / card mobile con serve · giacenza · da comprare · costo stimato e i cantieri che richiedono ogni articolo
...
- **Lista materiali da comprare (per lavoro)** — campo `lista_spesa` sul Lavoro:
  ...
  - `PATCH /api/lavori/{id}/lista/{item_id}` — modifica nome/quantita/unita/prezzo_stimato/comprato/note (404 voce inesistente). **Carico da lista**: se la voce ha `materiale_id`, spuntare `comprato=true` incrementa `quantita_disponibile` del materiale e marca `caricato=true`; togliere la spunta la decrementa; cambiare la quantità di una voce già caricata riallinea la giacenza col delta (idempotente grazie a `caricato`)
  - `GET /api/lavori/lista-spesa/unica` → `{righe[{chiave, materiale_id, nome, unita, quantita_richiesta, giacenza|null, mancante, prezzo_stimato, costo_stimato, cantieri[{lavoro_id,titolo,cliente_nome,quantita}]}], cantieri_aperti, totale_stimato}` — **lista della spesa unica**: unisce le voci non comprate di tutti i lavori con stato ≠ completato, raggruppando per materiale_id (o per nome normalizzato per le voci libere), sottraendo la giacenza e ordinando per mancante decrescente. La rotta è dichiarata **prima** di `GET /lavori/{lavoro_id}` per non essere catturata dal path param. Pagina `/spesa` (`pages/Spesa.tsx`, voce nav desktop+mobile «Spesa»), query key `lista-spesa-unica`
...
- `POST /api/materiali/ricarico-prezzi` {categoria ("tutte" o una delle 6), percentuale (-90…+500, ≠0), applica_a (vendita|costo|entrambi), arrotonda} — rincaro/sconto percentuale in blocco; risponde `{aggiornati, categoria, percentuale, applica_a, esempi}`. 422 percentuale 0/fuori scala o categoria non valida, 404 categoria senza materiali. **Operazione non annullabile** (per tornare indietro si applica la variazione inversa).
- `POST /api/materiali/carico-multiplo` {righe:[{materiale_id, quantita}]} — carico giacenze in blocco; risponde `{aggiornati, pezzi_totali, valore_carico, materiali}`. 422 se nessuna quantità, 404 se nessun id valido.
...
- `POST /api/preventivi/{id}/invia` — invia il preventivo per email al cliente e segna lo stato `inviato`, salvando `email_inviata_a` e `data_invio_email`. Il destinatario è letto dal documento salvato (`cliente_email`) e il corpo HTML deriva da un template server-side in `routers/preventivi.py` (`_corpo_email`): il client passa solo l'id, mai indirizzi o markup. L'email contiene un pulsante con il **link https al PDF** (`APP_URL` + token) — l'endpoint email gestito non supporta allegati veri, quindi il PDF viaggia come download di prima parte. 400 se il cliente non ha email.
- `GET /api/preventivi/{id}/pdf?t={token}` — PDF del preventivo generato con reportlab (`lib/pdf.py`). Con token presente deve combaciare con `pdf_token` salvato (403 altrimenti); senza token risponde comunque, perché l'app è personale e senza autenticazione.
...
- **Lavoro**: titolo, cliente_nome, cliente_telefono, cliente_indirizzo, descrizione, stato(da_iniziare|in_corso|completato), data_inizio, data_fine_prevista, prezzo_pattuito, ore_manodopera, note, materiali_usati[{materiale_id, nome, quantita, unita, prezzo_unitario}], ore_lavorate[{data, ore, tariffa_oraria, descrizione, preventivo_id}], lista_spesa[{materiale_id, nome, quantita, unita, prezzo_stimato, comprato, caricato, note}]
...
**PDF preventivi** — `backend/lib/pdf.py` (reportlab platypus, pinnato in requirements.txt): intestazione brand, dati cliente, tabelle materiali/manodopera, totali con sconto/IVA, condizioni e piè di pagina con numero di pagina. Verificato visivamente renderizzando le pagine (skill document-verification). `pymupdf` è solo tooling di verifica, **non** va in requirements.txt.
