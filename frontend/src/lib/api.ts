...
import { staticRequest, StaticApiError } from "@/lib/staticStore";

const BASE = "/api";

// Build statica (GitHub Pages): nessun backend, i dati vivono in localStorage.
...
async function request<T>(method: string, path: string, body?: JsonBody): Promise<T> {
  if (STATIC_MODE) {
    try {
      ...
      if (e instanceof StaticApiError) throw new ApiError(e.status, e.body);
      throw e;
    }
  }
...
// Upload multipart: il browser deve impostare da sé il boundary, quindi nessun header.
export async function apiUpload<T>(path: string, file: File): Promise<T> {
  if (STATIC_MODE) {
    throw new ApiError(501, {
      detail:
        "L'importazione del listino Excel non è disponibile nella versione statica (serve il server).",
    });
  }
